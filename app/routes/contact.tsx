...
export function meta({}: Route.MetaArgs) {
  return [
    ...
    { name: "description", content: "Reach out for AuDHD coaching support. No perfect prose required - just reach out however feels manageable today." },
  ];
}
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  try {
    ...
    return { 
      success: true, 
      message: "Thank you for reaching out! I'll get back to you within 48 hours (unless I'm in executive dysfunction jail, in which case I'll respond as soon as I escape)." 
    };
  } catch (error) {
...
function ContactForm() {
  ...
  return (
    <div>
      ...
      {isSuccess && (
        <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <BodyBase className="text-green-800">
                {successMessage} 
              </BodyBase>
            </div>
          </div>
          ...
          <div className="flex items-start space-x-3">
            ...
            <div>
              <BodyBase className="text-red-800">
                {errorMessage} <span>Please try again or email me directly at <ContactEmailLink/></span>
              </BodyBase>
            </div>
          </div>
        </div>
      ...
      <fetcher.Form ref={formRef} method="post" action="/contact" className="space-y-6">
        <div>
          ...
          <input 
            ...
            className={`w-full px-4 py-3 bg-surface border rounded-lg text-base transition-colors duration-200 placeholder:text-text-tertiary focus:outline-none focus:ring-4 focus:ring-primary/20 focus:border-primary disabled:opacity-50 disabled:cursor-not-allowed ${
              errors?.name ? 'border-red-300 focus:border-red-500 focus:ring-red-500/20' : 'border-border'
            }`}
        ...
        <div>
          ...
          <input 
            ...
            disabled={isFormDisabled}
            className={`w-full px-4 py-3 bg-surface border rounded-lg text-base transition-colors duration-200 placeholder:text-text-tertiary focus:outline-none focus:ring-4 focus:ring-primary/20 focus:border-primary disabled:opacity-50 disabled:cursor-not-allowed ${
              errors?.email ? 'border-red-300 focus:border-red-500 focus:ring-red-500/20' : 'border-border'
            }`}
        ...
        <div>
          <label htmlFor="message" className="block mb-2 text-sm font-medium text-text-secondary">
            Message (Can be as short as "Help. AuDHD. Drowning." I'll understand.)
          </label>
          <textarea 
            id="message" 
            name="message" 
            ...
            disabled={isFormDisabled}
            className={`w-full px-4 py-3 bg-surface border rounded-lg text-base transition-colors duration-200 placeholder:text-text-tertiary focus:outline-none focus:ring-4 focus:ring-primary/20 focus:border-primary disabled:opacity-50 disabled:cursor-not-allowed ${
              errors?.message ? 'border-red-300 focus:border-red-500 focus:ring-red-500/20' : 'border-border'
            }`}
        ...
        {isSuccess && (
          <BodySmall className="text-center text-text-secondary mt-3">
            Want to send another message? <button 
              type="button" 
    ...
    <>
      ...
      <Section>
        <Container>
          <div className="space-y-6 mb-12">
            <BodyBase>
              I know reaching out can feel overwhelming. The executive function required to write an email, the vulnerability of asking for help, the fear of being "too much" or "not autistic enough"—I get it.
            </BodyBase>
            <BodyBase>
              So let's keep this simple. No need for perfect prose or explaining your entire life story. Just reach out however feels manageable today.
            </BodyBase>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            ...
            <div>
              <H2 className="mb-6">Option 2: Book a Discovery Call</H2>
              <BodyBase className="text-text-secondary mb-6">
              ...
              <div className="bg-primary-soft rounded-xl p-6 md:p-8 border border-border transition-shadow duration-200 hover:shadow-sm">
                <H4 className="mb-4">Free 45-Minute Discovery Call/Vibe Check</H4>
                <ul className="space-y-2 mb-6 text-text-secondary">
                  ...
                  <li className="flex items-start space-x-3">
                    <span className="text-primary mt-1">•</span>
                    <BodySmall>Just two brains figuring out if we're a good match</BodySmall>
                  </li>
                </ul>
                <Link to="https://calendar.app.google/CSpmrAwmpipXxNkz8" target="_blank" className="w-full inline-flex items-center justify-center px-6 py-3 bg-primary text-white rounded-lg font-medium transition-colors duration-200 hover:bg-primary-hover focus:outline-none focus:ring-4 focus:ring-primary/20">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-surface rounded-xl p-6 md:p-8 border border-border transition-shadow duration-200 hover:shadow-sm">
              ...
              <BodyBase className="text-text-secondary">
                I'll respond within 48 hours (unless I'm in executive dysfunction jail, in which case I'll respond as soon as I escape). We'll figure out next steps together.
              </BodyBase>
            </div>
            <div className="bg-surface rounded-xl p-6 md:p-8 border border-border transition-shadow duration-200 hover:shadow-sm">
              <H4 className="mb-4">If you book a call:</H4>
              <BodyBase className="text-text-secondary">
                You'll get a confirmation email with our meeting link. Camera optional. Pajamas encouraged. Stimming welcome.
              </BodyBase>
            </div>
          </div>
        </Container>
      ...
      <Section>
        <Container>
          ...
          <BodyBase className="mb-6">Here are some conversation starters you can copy/paste:</BodyBase>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-surface border border-border rounded-lg p-4">
              <BodySmall className="text-text-secondary font-mono">
                "Hi Francois, I just got diagnosed and I have no idea what to do next."
              </BodySmall>
            </div>
            <div className="bg-surface border border-border rounded-lg p-4">
              <BodySmall className="text-text-secondary font-mono">
                "I think I might be AuDHD but I'm drowning in imposter syndrome."
              </BodySmall>
            </div>
            <div className="bg-surface border border-border rounded-lg p-4">
              <BodySmall className="text-text-secondary font-mono">
                "I need help with [specific thing] and traditional advice isn't working."
              </BodySmall>
            </div>
            <div className="bg-surface border border-border rounded-lg p-4">
              <BodySmall className="text-text-secondary font-mono">
      ...
      <Section background="primary-soft">
        <Container>
          ...
          <ul className="space-y-4">
            <li className="flex items-start space-x-3">
              <span className="text-primary mt-1">•</span>
              <BodyBase>No judgment about how long it took you to reach out</BodyBase>
            </li>
            <li className="flex items-start space-x-3">
              <span className="text-primary mt-1">•</span>
            ...
            <li className="flex items-start space-x-3">
              <span className="text-primary mt-1">•</span>
              <BodyBase>No neurotypical "solutions" that don't actually work</BodyBase>
            </li>
            <li className="flex items-start space-x-3">
              <span className="text-primary mt-1">•</span>
            ...
            <li className="flex items-start space-x-3">
              <span className="text-primary mt-1">•</span>
              <BodyBase>Genuine understanding from someone who's been there</BodyBase>
            </li>
          </ul>
        </Container>
      </Section>
        ...
        <Container>
          <div className="text-center">
            ...
            <BodySmall className="text-text-tertiary">
              <em>Remember: Asking for help isn't weakness. It's the first step toward building a life that actually fits your brain.</em>
            </BodySmall>
          </div>
        </Container>
      </Section>
